...
- 引用 exploration.md 的冲突点检测结论（§4，确认 spec 范围不与已确认规则矛盾）

**§2 用户价值（用 Job-to-be-Done 框架分析）：**
...
**§5 验收标准（必须符合 SMART 原则 + AC-NNN 表格化）：**
...
验收标准格式（AC-NNN 表格化，追踪链 ID 规范见 standards/sdd/traceability-ids.md）：
```
| ID     | 验收标准（可测试）                                              | 备注 |
| ------ | --------------------------------------------------------------- | ---- |
| AC-001 | 用户输入有效邮箱和密码 → 点击注册 → 注册成功 → 跳转到首页      |      |
| AC-002 | 用户输入已注册邮箱 → 点击注册 → 显示"邮箱已注册" → 提供找回密码 |      |
| AC-003 | 用户输入无效邮箱格式 → 点击注册 → 显示"邮箱格式不正确" → 不提交 |      |
```

**§6 成功指标（Success Metrics，业界标配）：**
- 验收标准管"能用"，成功指标管"有用/值不值"
- 给出可观测指标与预期值（如转化率、耗时、错误率）；无法度量时写"本期不度量"并说明原因
...
- [ ] 每条业务规则是否有明确的条件→结果？
- [ ] §5 验收标准是否为 AC-NNN 表格（AC-001 起三位递增），每条可测试（有输入/输出）？
- [ ] §6 成功指标是否给出可观测指标或明确标注"本期不度量"？
...
**spec §5 验收标准节选（AC-NNN 表格化）：**
```
| ID     | 验收标准（可测试）                                        | 备注 |
| ------ | --------------------------------------------------------- | ---- |
| AC-001 | 有效邮箱+有效密码 → 注册成功 → 跳转首页                  |      |
| AC-002 | 已注册邮箱 → 提示"邮箱已注册" → 提供找回密码链接        |      |
| AC-003 | 无效邮箱格式 → 提示"邮箱格式不正确" → 不提交            |      |
| AC-004 | 有效手机号+有效密码 → 注册成功 → 跳转首页                |      |
| AC-005 | 密码强度不足 → 提示强度要求 → 不提交                    |      |
```
...
- AC 编号（AC-001、AC-002…三位递增，规范见 standards/sdd/traceability-ids.md）一旦确认保持稳定且不复用——design covers / TC verified-by / task 阶段 DU Acceptance 均按编号引用，避免跨仓验收失联
