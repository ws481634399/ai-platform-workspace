...
|---|---|---|---|---|
| TC-001 | Unit/集成测试 | AC-001 | DU-BE-209 | 验证 每类关键成功变更均生成包含 operator/action/target/result/traceId 的审计记录。 |
| TC-002 | API/安全边界测试 | AC-002 | DU-BE-209 | 验证 失败的防锁死或非法分配操作也记录失败原因分类，且不含敏感值。 |
| TC-003 | API/安全边界测试 | AC-003 | DU-BE-209 | 验证 普通管理员管理 API 无法修改或删除既有审计记录。 |
