...
- 涉及仓库: repo-1
- 模块改动摘要: mall-identity/rbac 新增 Menu 聚合、环检测、树查询与管理 Controller。

### 1.1 repo-1

- mall-identity/rbac 新增 Menu 聚合、环检测、树查询与管理 Controller。

## 2. 接口契约细化

MenuDto 使用稳定 componentKey；树服务按 sort/id 排序并过滤无效后代。 跨 Story 共享类型与边界引用 `change-design.md` §2、§4、§5.1，不在本 Story 另造不兼容协议。

## 3. 数据变更

- 是否需 Migration: yes
- 变更摘要: Flyway V2 创建 menu 表、parent_id/sort 索引和 code 唯一约束。
...
|---|---|---|---|---|
| DU-BE-204 | repo-1 | DES-001：实现本 Story 在 repo-1 的职责 | AC-001, AC-002, AC-003 | — |
