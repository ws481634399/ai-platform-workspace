...
- 涉及仓库: repo-1
- 模块改动摘要: mall-identity 新增 AuthenticationApplicationService、LoginController、登录请求校验与认证审计事件。

### 1.1 repo-1

- mall-identity 新增 AuthenticationApplicationService、LoginController、登录请求校验与认证审计事件。

## 2. 接口契约细化

POST /api/admin/auth/login 接收 username/password；统一认证失败码，不把查询结果差异暴露给客户端。 跨 Story 共享类型与边界引用 `change-design.md` §2、§4、§5.1，不在本 Story 另造不兼容协议。

## 3. 数据变更

- 是否需 Migration: no
- 变更摘要: 无新增表，复用 admin_user。
...
|---|---|---|---|---|
| DU-BE-102 | repo-1 | DES-001：实现本 Story 在 repo-1 的职责 | AC-001, AC-002, AC-003 | — |
