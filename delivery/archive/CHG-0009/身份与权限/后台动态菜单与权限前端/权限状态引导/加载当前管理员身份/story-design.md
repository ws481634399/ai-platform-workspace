...
- 涉及仓库: repo-1、repo-2
- 模块改动摘要: repo-1 提供 bootstrap user DTO；repo-2 新增 sessionApi 与 permission store 身份状态。

### 1.1 repo-1

- repo-1 提供 bootstrap user DTO；repo-2 新增 sessionApi 与 permission store 身份状态。
...
GET /api/admin/session/bootstrap 返回 user；前端 401/403/网络错误分流。 跨 Story 共享类型与边界引用 `change-design.md` §2、§4、§5.1，不在本 Story 另造不兼容协议。
...
|---|---|---|---|---|
| DU-BE-301 | repo-1 | DES-001：实现本 Story 在 repo-1 的职责 | AC-001, AC-002, AC-003 | — |
| DU-FE-301 | repo-2 | DES-002：实现本 Story 在 repo-2 的职责 | AC-001, AC-002, AC-003 | DU-BE-301 |
