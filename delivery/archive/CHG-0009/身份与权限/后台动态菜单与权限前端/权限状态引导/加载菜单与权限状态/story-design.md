...
- 涉及仓库: repo-1、repo-2
- 模块改动摘要: repo-1 聚合 MenuDto/permissions/version；repo-2 完成 DTO 校验、permissions Set 与共享 bootstrap Promise。

### 1.1 repo-1

- repo-1 聚合 MenuDto/permissions/version；repo-2 完成 DTO 校验、permissions Set 与共享 bootstrap Promise。
...
SessionBootstrap 字段严格对齐 change-design §4，非法菜单不进入路由输入。 跨 Story 共享类型与边界引用 `change-design.md` §2、§4、§5.1，不在本 Story 另造不兼容协议。
...
|---|---|---|---|---|
| DU-BE-302 | repo-1 | DES-001：实现本 Story 在 repo-1 的职责 | AC-001, AC-002, AC-003 | — |
| DU-FE-302 | repo-2 | DES-002：实现本 Story 在 repo-2 的职责 | AC-001, AC-002, AC-003 | DU-BE-302 |
