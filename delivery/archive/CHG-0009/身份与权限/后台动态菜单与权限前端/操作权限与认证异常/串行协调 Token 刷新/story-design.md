...
- 涉及仓库: repo-2
- 模块改动摘要: mall-admin 重构 api/http.ts，新增 RefreshCoordinator、Axios 配置扩展和一次重放。

### 1.1 repo-2

- mall-admin 重构 api/http.ts，新增 RefreshCoordinator、Axios 配置扩展和一次重放。

## 2. 接口契约细化

仅特定 401 触发 refreshOnce；skipAuthRefresh/_retry 防递归。 跨 Story 共享类型与边界引用 `change-design.md` §2、§4、§5.1，不在本 Story 另造不兼容协议。
...
|---|---|---|---|---|
| DU-FE-308 | repo-2 | DES-001：实现本 Story 在 repo-2 的职责 | AC-001, AC-002, AC-003 | — |
