...
- 涉及仓库: repo-2
- 模块改动摘要: mall-admin 新增 guards/permission.ts、ForbiddenView，并重构 router 静态路由与 catch-all。

### 1.1 repo-2

- mall-admin 新增 guards/permission.ts、ForbiddenView，并重构 router 静态路由与 catch-all。

## 2. 接口契约细化

beforeEach 使用返回值签名；公开/401/bootstrap/403/404 顺序固定。 跨 Story 共享类型与边界引用 `change-design.md` §2、§4、§5.1，不在本 Story 另造不兼容协议。
...
|---|---|---|---|---|
| DU-FE-306 | repo-2 | DES-001：实现本 Story 在 repo-2 的职责 | AC-001, AC-002, AC-003 | — |
