...
|---|---|---|---|---|
| TC-001 | 组件/E2E 测试 | AC-001 | DU-FE-306 | 验证 未登录访问受保护路由时跳转登录页并保留原目标 redirect。 |
| TC-002 | 组件/E2E 测试 | AC-002 | DU-FE-306 | 验证 已登录但缺少 route permission 时进入 403，认证状态不被清除。 |
| TC-003 | 组件/E2E 测试 | AC-003 | DU-FE-306 | 验证 访问未知路径时进入 404；守卫使用返回值式签名且浏览器刷新行为一致。 |
