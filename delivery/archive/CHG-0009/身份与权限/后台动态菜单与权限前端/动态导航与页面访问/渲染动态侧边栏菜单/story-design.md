...
- 涉及仓库: repo-2
- 模块改动摘要: mall-admin 新增 DynamicMenu/MenuTreeItem 并改造 AdminLayout 从 permission store 渲染。

### 1.1 repo-2

- mall-admin 新增 DynamicMenu/MenuTreeItem 并改造 AdminLayout 从 permission store 渲染。

## 2. 接口契约细化

MenuNode 文本转义、图标白名单和 sort/id 稳定排序。 跨 Story 共享类型与边界引用 `change-design.md` §2、§4、§5.1，不在本 Story 另造不兼容协议。
...
|---|---|---|---|---|
| DU-FE-304 | repo-2 | DES-001：实现本 Story 在 repo-2 的职责 | AC-001, AC-002, AC-003 | — |
