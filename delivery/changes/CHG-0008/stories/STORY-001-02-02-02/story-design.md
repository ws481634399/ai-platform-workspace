...
- 涉及仓库: repo-1
- 模块改动摘要: mall-identity/rbac 新增 Permission 实体、编码值对象、Repository 和管理 Controller。

### 1.1 repo-1

- mall-identity/rbac 新增 Permission 实体、编码值对象、Repository 和管理 Controller。

## 2. 接口契约细化

PermissionDto 与 code/type/httpMethod/pathPattern 校验遵循 story-spec。 跨 Story 共享类型与边界引用 `change-design.md` §2、§4、§5.1，不在本 Story 另造不兼容协议。

## 3. 数据变更

- 是否需 Migration: yes
- 变更摘要: Flyway V2 创建 permission 表与 code 唯一索引。
...
|---|---|---|---|---|
| DU-BE-205 | repo-1 | DES-001：实现本 Story 在 repo-1 的职责 | AC-001, AC-002, AC-003 | — |
