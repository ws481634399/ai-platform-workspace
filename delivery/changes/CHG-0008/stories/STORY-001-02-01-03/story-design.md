...
- 涉及仓库: repo-1
- 模块改动摘要: mall-identity/rbac 新增 AdminRoleAssignmentService、关联 Repository 与权限版本失效端口。

### 1.1 repo-1

- mall-identity/rbac 新增 AdminRoleAssignmentService、关联 Repository 与权限版本失效端口。

## 2. 接口契约细化

PUT /admins/{id}/roles 完整替换，先校验全集再事务写入。 跨 Story 共享类型与边界引用 `change-design.md` §2、§4、§5.1，不在本 Story 另造不兼容协议。

## 3. 数据变更

- 是否需 Migration: yes
- 变更摘要: Flyway V2 创建 admin_user_role 联合唯一键与外键/逻辑约束。
...
|---|---|---|---|---|
| DU-BE-203 | repo-1 | DES-001：实现本 Story 在 repo-1 的职责 | AC-001, AC-002, AC-003 | — |
