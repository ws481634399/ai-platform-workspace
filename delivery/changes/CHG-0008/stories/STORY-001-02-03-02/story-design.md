...
- 涉及仓库: repo-1
- 模块改动摘要: mall-identity 新增 AuthorizationSnapshotQueryService、RedisPermissionCache 与 TransactionalPermissionInvalidator。

### 1.1 repo-1

- mall-identity 新增 AuthorizationSnapshotQueryService、RedisPermissionCache 与 TransactionalPermissionInvalidator。

## 2. 接口契约细化

缓存 key/TTL/回源遵循 change-design §2.3；Redis 异常走数据库，不默认放行。 跨 Story 共享类型与边界引用 `change-design.md` §2、§4、§5.1，不在本 Story 另造不兼容协议。

## 3. 数据变更

- 是否需 Migration: no
- 变更摘要: 复用 permission_version 与关系表。
...
|---|---|---|---|---|
| DU-BE-208 | repo-1 | DES-001：实现本 Story 在 repo-1 的职责 | AC-001, AC-002, AC-003 | — |
