...
- 涉及仓库: repo-1
- 模块改动摘要: mall-identity/rbac 新增 Role 实体、Repository、应用服务与 Controller。

### 1.1 repo-1

- mall-identity/rbac 新增 Role 实体、Repository、应用服务与 Controller。

## 2. 接口契约细化

角色 code 在领域层和数据库双重唯一；内置角色字段不可由 DTO 覆盖。 跨 Story 共享类型与边界引用 `change-design.md` §2、§4、§5.1，不在本 Story 另造不兼容协议。

## 3. 数据变更

- 是否需 Migration: yes
- 变更摘要: Flyway V2 创建 role 表与 code 唯一索引。
...
|---|---|---|---|---|
| DU-BE-202 | repo-1 | DES-001：实现本 Story 在 repo-1 的职责 | AC-001, AC-002, AC-003 | — |
