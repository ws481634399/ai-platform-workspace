...
- 涉及仓库: repo-1
- 模块改动摘要: mall-identity/rbac 新增 RoleGrantAssignmentService、关系 Repository、影响管理员查询与失效端口。

### 1.1 repo-1

- mall-identity/rbac 新增 RoleGrantAssignmentService、关系 Repository、影响管理员查询与失效端口。

## 2. 接口契约细化

PUT /roles/{id}/grants 在事务内完整替换 menuIds/permissionIds。 跨 Story 共享类型与边界引用 `change-design.md` §2、§4、§5.1，不在本 Story 另造不兼容协议。

## 3. 数据变更

- 是否需 Migration: yes
- 变更摘要: Flyway V2 创建 role_menu、role_permission 联合唯一键。
...
|---|---|---|---|---|
| DU-BE-206 | repo-1 | DES-001：实现本 Story 在 repo-1 的职责 | AC-001, AC-002, AC-003 | — |
