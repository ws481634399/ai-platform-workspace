...
|---|---|---|---|---|
| TC-001 | Unit/集成测试 | AC-001 | DU-BE-206 | 验证 有效授权集合替换成功后，角色菜单与权限关系和请求完全一致。 |
| TC-002 | API/安全边界测试 | AC-002 | DU-BE-206 | 验证 请求包含不存在或禁用资源时事务回滚，原授权不变。 |
| TC-003 | API/安全边界测试 | AC-003 | DU-BE-206 | 验证 角色授权变化后所有关联管理员 permissionVersion 递增并失效缓存。 |
