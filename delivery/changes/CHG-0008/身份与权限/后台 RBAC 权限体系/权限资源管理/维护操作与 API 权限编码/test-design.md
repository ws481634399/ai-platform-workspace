...
|---|---|---|---|---|
| TC-001 | Unit/集成测试 | AC-001 | DU-BE-205 | 验证 合法 BUTTON/API 权限可创建查询，非法或重复 code 返回 400/409。 |
| TC-002 | API/安全边界测试 | AC-002 | DU-BE-205 | 验证 禁用权限后授权快照不再包含该 code。 |
| TC-003 | API/安全边界测试 | AC-003 | DU-BE-205 | 验证 修改权限名称或资源描述不改变其稳定 code，尝试修改 code 被拒绝。 |
