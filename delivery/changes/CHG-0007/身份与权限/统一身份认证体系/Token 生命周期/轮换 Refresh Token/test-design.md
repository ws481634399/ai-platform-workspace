...
|---|---|---|---|---|
| TC-001 | Unit/集成测试 | AC-001 | DU-BE-104 | 验证 给定有效 Refresh Token，当首次刷新时，旧会话被标记已轮换并返回全新 TokenPair。 |
| TC-002 | API/安全边界测试 | AC-002 | DU-BE-104 | 验证 给定已轮换、撤销或过期 Refresh Token，当刷新时，返回 401 且不生成新会话。 |
| TC-003 | API/安全边界测试 | AC-003 | DU-BE-104 | 验证 两个并发请求使用同一 Refresh Token 时，最多一个成功，另一个按重放策略失败并留下可审计结果。 |
