...
- 涉及仓库: repo-1
- 模块改动摘要: mall-common-security 新增 SubjectType、AuthenticatedSubject、JwtClaimNames 和受众/类型校验器。

### 1.1 repo-1

- mall-common-security 新增 SubjectType、AuthenticatedSubject、JwtClaimNames 和受众/类型校验器。

## 2. 接口契约细化

JWT converter 严格读取受控 claims，未知 subject_type 或 audience 直接失败。 跨 Story 共享类型与边界引用 `change-design.md` §2、§4、§5.1，不在本 Story 另造不兼容协议。
...
|---|---|---|---|---|
| DU-BE-106 | repo-1 | DES-001：实现本 Story 在 repo-1 的职责 | AC-001, AC-002, AC-003 | — |
