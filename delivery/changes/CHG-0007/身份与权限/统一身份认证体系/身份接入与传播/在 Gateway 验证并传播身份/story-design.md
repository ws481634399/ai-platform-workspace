...
- 涉及仓库: repo-1
- 模块改动摘要: mall-gateway 新增 WebFlux JwtDecoder、GlobalFilter、内部头常量与 JSON 401 handler。

### 1.1 repo-1

- mall-gateway 新增 WebFlux JwtDecoder、GlobalFilter、内部头常量与 JSON 401 handler。

## 2. 接口契约细化

Filter 先清内部头，再验证 JWT 并写规范头，同时保留 Authorization。 跨 Story 共享类型与边界引用 `change-design.md` §2、§4、§5.1，不在本 Story 另造不兼容协议。
...
|---|---|---|---|---|
| DU-BE-108 | repo-1 | DES-001：实现本 Story 在 repo-1 的职责 | AC-001, AC-002, AC-003 | — |
