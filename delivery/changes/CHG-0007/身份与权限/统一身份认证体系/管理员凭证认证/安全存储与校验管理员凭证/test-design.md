...
|---|---|---|---|---|
| TC-001 | Unit/集成测试 | AC-001 | DU-BE-101 | 验证 给定合法用户名和密码，当创建凭证时，数据库仅保存不可逆强哈希且能正确匹配原密码。 |
| TC-002 | API/安全边界测试 | AC-002 | DU-BE-101 | 验证 给定 DISABLED 管理员，当校验登录资格时，返回不可登录结果且不执行 Token 签发。 |
| TC-003 | API/安全边界测试 | AC-003 | DU-BE-101 | 验证 对管理员响应与普通日志执行敏感字段扫描时，不出现 password、passwordHash、完整 Token 或密钥。 |
