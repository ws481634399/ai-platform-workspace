...
|---|---|---|---|---|
| TC-001 | Unit/集成测试 | AC-001 | DU-BE-105 | 验证 给定有效会话，当主动退出时，会话被撤销且同一 Refresh Token 再刷新返回 401。 |
| TC-002 | API/安全边界测试 | AC-002 | DU-BE-105 | 验证 重复提交同一退出请求时，返回幂等成功且不创建额外会话。 |
| TC-003 | API/安全边界测试 | AC-003 | DU-BE-105 | 验证 管理员被禁用后，新登录和 Refresh 均失败，旧会话版本不能继续换取凭证。 |
