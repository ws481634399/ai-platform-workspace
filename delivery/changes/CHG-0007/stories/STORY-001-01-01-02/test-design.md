...
|---|---|---|---|---|
| TC-001 | Unit/集成测试 | AC-001 | DU-BE-102 | 验证 给定启用管理员和正确密码，当调用登录接口时，返回成功认证结果并进入双 Token 签发。 |
| TC-002 | API/安全边界测试 | AC-002 | DU-BE-102 | 验证 给定不存在账号或错误密码，当调用登录接口时，均返回相同 401 业务语义且不泄露账号是否存在。 |
| TC-003 | API/安全边界测试 | AC-003 | DU-BE-102 | 验证 给定禁用管理员，当调用登录接口时，返回 401 且不创建 Refresh Session。 |
