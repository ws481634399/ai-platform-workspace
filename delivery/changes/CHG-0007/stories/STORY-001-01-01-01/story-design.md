...
- 涉及仓库: repo-1
- 模块改动摘要: mall-identity 新增 AdminCredential 实体、Mapper/Repository、PasswordEncoder 配置与 DTO 映射。

### 1.1 repo-1

- mall-identity 新增 AdminCredential 实体、Mapper/Repository、PasswordEncoder 配置与 DTO 映射。

## 2. 接口契约细化

按 story-spec §4 实现 AdminCredential 字段；密码写入前编码，查询 DTO 永不映射 passwordHash。 跨 Story 共享类型与边界引用 `change-design.md` §2、§4、§5.1，不在本 Story 另造不兼容协议。

## 3. 数据变更

- 是否需 Migration: yes
- 变更摘要: Flyway V1 创建 admin_user（username 唯一、password_hash、status、auth_version、permission_version、审计字段）。
...
|---|---|---|---|---|
| DU-BE-101 | repo-1 | DES-001：实现本 Story 在 repo-1 的职责 | AC-001, AC-002, AC-003 | — |
