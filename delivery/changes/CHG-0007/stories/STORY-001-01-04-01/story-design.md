...
- 涉及仓库: repo-2
- 模块改动摘要: mall-admin 新增 api/auth.ts、stores/auth.ts，替换 LoginView 占位并接入基础路由跳转。

### 1.1 repo-2

- mall-admin 新增 api/auth.ts、stores/auth.ts，替换 LoginView 占位并接入基础路由跳转。

## 2. 接口契约细化

TypeScript TokenPair/LoginRequest 对齐后端；storage 只由 auth store 访问。 跨 Story 共享类型与边界引用 `change-design.md` §2、§4、§5.1，不在本 Story 另造不兼容协议。
...
|---|---|---|---|---|
| DU-FE-101 | repo-2 | DES-001：实现本 Story 在 repo-2 的职责 | AC-001, AC-002, AC-003 | — |
