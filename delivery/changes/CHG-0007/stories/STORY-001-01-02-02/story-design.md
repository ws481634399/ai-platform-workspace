...
- 涉及仓库: repo-1
- 模块改动摘要: mall-identity 新增 RefreshTokenApplicationService 和行锁查询，原子轮换 Session。

### 1.1 repo-1

- mall-identity 新增 RefreshTokenApplicationService 和行锁查询，原子轮换 Session。

## 2. 接口契约细化

POST /api/admin/auth/refresh；事务锁定 token_hash，校验后标记 rotated_to 并生成新 Session。 跨 Story 共享类型与边界引用 `change-design.md` §2、§4、§5.1，不在本 Story 另造不兼容协议。

## 3. 数据变更

- 是否需 Migration: no
- 变更摘要: 复用 auth_session；需要 token_hash 与 family_id 索引。
...
|---|---|---|---|---|
| DU-BE-104 | repo-1 | DES-001：实现本 Story 在 repo-1 的职责 | AC-001, AC-002, AC-003 | — |
