...
- 涉及仓库: repo-1
- 模块改动摘要: mall-common-security 新增 Servlet Resource Server 自动配置、CurrentSubjectAccessor、统一 401/403 handler。

### 1.1 repo-1

- mall-common-security 新增 Servlet Resource Server 自动配置、CurrentSubjectAccessor、统一 401/403 handler。

## 2. 接口契约细化

Authorization Bearer 经 JwtDecoder 验证后映射 principal；公开路径由应用显式提供。 跨 Story 共享类型与边界引用 `change-design.md` §2、§4、§5.1，不在本 Story 另造不兼容协议。
...
|---|---|---|---|---|
| DU-BE-107 | repo-1 | DES-001：实现本 Story 在 repo-1 的职责 | AC-001, AC-002, AC-003 | — |
