...
共 11 个 Task，按依赖顺序执行；TASK-008~011 为验证与证据任务，粒度上各自独立成 Task 以保证 AC 可追溯。

### TASK-001: 创建 backend 根聚合 POM

- 目标仓库: repo-1
- 目标模块: backend/pom.xml
...
- 验证方法: POM 结构审查（本 Task 完成后 backend 全量构建尚不可用，首个构建验证点在 TASK-008）
- 依赖: 无
- 预估变更: ~120 行
...
- 目标仓库: repo-1
- 目标模块: backend/mall-services/mall-{identity,member,product,cart,order,inventory,search,system}/
...
### TASK-008: 根目录全量构建验证（覆盖 AC-1、AC-2）

- 目标仓库: repo-1
- 目标模块: backend/
- 预期变更: 无代码变更（验证任务；如遇失败做最小修复）
- 验证方法: 在 backend/ 执行 `mvn clean package -DskipTests` → BUILD SUCCESS；核对 Reactor 摘要包含全部 24 个 Maven 项目、无模块缺失/artifactId 冲突；输出留档
...
- 目标仓库: repo-1
- 目标模块: backend/mall-gateway/、backend/mall-services/*
- 预期变更: 无代码变更（验证任务）
...
- 目标仓库: repo-1
- 目标模块: delivery/changes/CHG-0001/evidence/、backend/
- 预期变更: 新增 evidence/build-evidence.md（按需求 §21 模板：Requirement=ENG-BASE-001、Java=21、Maven 版本、Build Command、Result=PASS/FAIL、Modules 实际清单、Notes）；新增 backend/README.md（构建说明 + JDK 21/Maven 3.9+ 环境要求 + 常用命令）；新增根 .gitignore（target/ 等 Maven 忽略项）
