...
- 涉及仓库: repo-2
- 模块改动摘要: mall-admin 新增 permissionBootstrap 编排、session restore、refreshPermissions 与动态路由差异同步。

### 1.1 repo-2

- mall-admin 新增 permissionBootstrap 编排、session restore、refreshPermissions 与动态路由差异同步。

## 2. 接口契约细化

恢复流程按 auth restore → bootstrap → route sync 顺序，失败时不留下部分状态。 跨 Story 共享类型与边界引用 `change-design.md` §2、§4、§5.1，不在本 Story 另造不兼容协议。
...
|---|---|---|---|---|
| DU-FE-303 | repo-2 | DES-001：实现本 Story 在 repo-2 的职责 | AC-001, AC-002, AC-003 | — |
