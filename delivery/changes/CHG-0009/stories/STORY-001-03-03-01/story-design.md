...
- 涉及仓库: repo-2
- 模块改动摘要: mall-admin 新增 composables/usePermission 与 directives/permission，共享纯 hasPermission。

### 1.1 repo-2

- mall-admin 新增 composables/usePermission 与 directives/permission，共享纯 hasPermission。

## 2. 接口契约细化

Directive 支持默认 remove/隐藏和 disabled modifier，响应权限 store 更新。 跨 Story 共享类型与边界引用 `change-design.md` §2、§4、§5.1，不在本 Story 另造不兼容协议。
...
|---|---|---|---|---|
| DU-FE-307 | repo-2 | DES-001：实现本 Story 在 repo-2 的职责 | AC-001, AC-002, AC-003 | — |
