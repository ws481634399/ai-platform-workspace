...
- 涉及仓库: repo-2
- 模块改动摘要: mall-admin 新增 sessionLifecycle.clearSession、HTTP 错误分类与 Forbidden 用户体验。

### 1.1 repo-2

- mall-admin 新增 sessionLifecycle.clearSession、HTTP 错误分类与 Forbidden 用户体验。

## 2. 接口契约细化

401 不可刷新时集中清理并安全跳转；403 保留会话；清理幂等。 跨 Story 共享类型与边界引用 `change-design.md` §2、§4、§5.1，不在本 Story 另造不兼容协议。
...
|---|---|---|---|---|
| DU-FE-309 | repo-2 | DES-001：实现本 Story 在 repo-2 的职责 | AC-001, AC-002, AC-003 | — |
