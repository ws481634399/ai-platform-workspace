...
opencode 已生成项目级斜杠命令（`.opencode/commands/`），优先使用：

```
/sdd-explore <CHG-ID> | /sdd-prd <CHG-ID> | /sdd-design <CHG-ID> | /sdd-task <CHG-ID>
/sdd-dev <CHG-ID>（多仓需带 --du） | /sdd-test <CHG-ID> | /sdd-review <CHG-ID> | /sdd-converge <CHG-ID>
```
